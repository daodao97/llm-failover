...
- `third` 渠道，会把单个渠道内每一次真实上游失败计入熔断窗口
- `pool` 渠道，按整轮 key 池尝试的最终结果计入熔断窗口

而不是所有渠道都统一等整轮渠道尝试彻底结束后才记一笔。

这样做的原因很现实：

- 如果一个 `third` 渠道在一次请求里已经连续 429 / 5xx 多次，就不该再等几轮用户请求后才判定它坏了
- 熔断器应该对真实上游失败敏感，而不是只对“最终结果”敏感
- 这样可以更早打开熔断，减少坏渠道把整条请求链路拖长

之所以对 `pool` 保守处理，是因为账号池里一个 key 失败，不代表池里别的 key 也失败。
对这类渠道，更合理的健康语义是“这次池内轮换后最终是否成功”。
...
- 不会在同一次半开探测里继续做多次同 key 重试
- 一旦第一次探测失败，会立刻重新打开熔断

这样可以避免“同一次探测里前几次失败，后面一次侥幸成功”导致渠道被误判为恢复。
...
- `ProbeTimeout`：单次半开探测的在途时长上限（默认 1 分钟）。探测超时未决时视同放弃，后续请求可重新探测，渠道不会卡死在半开状态。
- `MaxCooldown`：连续熔断时指数退避冷却的上限（默认 16 倍 `Cooldown`），避免长故障后恢复探测被推迟数小时。
- `MaxWindowSamples`：失败窗口内保留的事件条数上限（默认 2048），防止高 QPS 渠道窗口状态无限增长。

另外，如果你的服务会动态创建大量 `Proxy`（例如按租户构建），
不再使用时应调用 `proxy.Close()`，将其熔断器从全局健康快照注册表中注销。
