...
如果第一次探测失败，立刻重新进入 `open`，不会在同一轮半开探测里继续做多次重试。

探测还有第三种结局：失败但不计入熔断（例如 4xx 不记账、客户端取消）。
这类"无结论"的探测会把半开标记复位，让下一个请求重新发起探测，
而不是让渠道永久卡在半开状态被跳过。

此外每次探测都带有一个 deadline（`ProbeTimeout`，默认 1 分钟）。
如果一个探测在 deadline 内既没有记成功也没有记失败（例如处理探测响应的
钩子 panic），后续请求会把它视同放弃并重新发起探测。
这保证状态机对任何未解决的探测都能自愈，而不依赖调用方枚举所有退出路径。

另外，熔断窗口统计会区分渠道类型：

- `third` 渠道按单次真实失败记账，这样在一个请求内部出现连续 429 / 5xx 时，熔断器就能更早感知
- `pool` 渠道按整轮 key 池尝试的最终结果记账，因为一个 key 失败不等于整个池失败
